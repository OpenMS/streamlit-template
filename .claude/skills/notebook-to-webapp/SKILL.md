...
1. **Set up everything that needs setting up, before the first question.**
   ...
   **Preflight checklist, all of it before the first question:**
   ...
   3. **Look for a Chromium-based browser** — Chrome, Edge, Brave, Chromium.
      Windows keeps them under `Program Files`; elsewhere try the executable
      name on `PATH`. Edge ships with Windows, so on a Windows machine the
      answer is almost always yes.
   4. **Connect browser control, here, by using it.** Its tools are deferred MCP
      ...
      **Check that it started; `start` on Windows does not.** A measured run
      ...
      The same applies later: **opening a page starts a browser**, so if you
      open the app for the user before control is established, probe again
      immediately — a browser you just launched can be driven.
      ...
      *"Can't interact with browser-internal or unparseable URLs"*, so a blank
      ...
      the table above.
   5. Install the gate's headless browser.
